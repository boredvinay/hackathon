...
export class CanvasComponent {
  ...
  // toolbar should call canvasComp.onDragStart(event, type, subtype)
  ...
  addDirectly(type: string, subtype: string) {
    // Map toolbar identifiers to element payload your addElement expects
    const base = { x: this.paper.x() + 20, y: this.paper.y() + 20 };
    if (type === 'shape') {
      this.addElement({ ...base, type: 'shape', shape: subtype });
    } else if (type === 'widget') {
      if (subtype === 'text') {
        this.addElement({ ...base, type: 'widget', widget: 'text', text: 'Edit me', placeholder: '' });
      } else if (subtype === 'barcode') {
        this.addElement({ ...base, type: 'widget', widget: 'barcode', barcodeType: 'Code128', value: 'sample123' });
      } else if (subtype === 'qrcode') {
        this.addElement({ ...base, type: 'widget', widget: 'qrcode', barcodeType: 'QRCode', value: 'https://example.com' });
      } else if (subtype === 'datamatrix') {
        this.addElement({ ...base, type: 'widget', widget: 'datamatrix', barcodeType: 'DataMatrix', value: 'DM123' });
      } else if (subtype === 'aztec') {
        this.addElement({ ...base, type: 'widget', widget: 'aztec', barcodeType: 'Aztec', value: 'AZ123' });
      } else if (subtype === 'maxicode') {
        this.addElement({ ...base, type: 'widget', widget: 'maxicode', barcodeType: 'MaxiCode', value: 'MX123' });
      }
    }
  }
  ...
  private onStageClick(e: Konva.KonvaEventObject<MouseEvent>) {
    if (e.target === this.stage || e.target === this.paper) return this.clearSelection();
  ...
  private onDrop(e: DragEvent) {
    ...
    if (elem.type && elem.subtype) {
      if (elem.type === 'shape') this.addElement({ type: 'shape', shape: elem.subtype, x, y });
      else if (elem.type === 'widget') {
        const w = elem.subtype;
        if (w === 'text') this.addElement({ type: 'widget', widget: 'text', text: 'Edit me', x, y, placeholder: '' });
        else if (w === 'barcode') this.addElement({ type: 'widget', widget: 'barcode', barcodeType: 'Code128', value: 'sample123', x, y });
        else if (w === 'qrcode') this.addElement({ type: 'widget', widget: 'qrcode', barcodeType: 'QRCode', value: 'https://example.com', x, y });
        else if (w === 'datamatrix') this.addElement({ type: 'widget', widget: 'datamatrix', barcodeType: 'DataMatrix', value: 'DM123', x, y });
        else if (w === 'aztec') this.addElement({ type: 'widget', widget: 'aztec', barcodeType: 'Aztec', value: 'AZ123', x, y });
        else if (w === 'maxicode') this.addElement({ type: 'widget', widget: 'maxicode', barcodeType: 'MaxiCode', value: 'MX123', x, y });
      }
      return;
    }
  ...
  private getDragBoundFunc(node: Konva.Shape) {
    return (pos: Konva.Vector2d) => {
      ...
      const maxY = this.paper.y() + this.paper.height() - (node.height?.() ?? 0);
      return {
  ...
  addElement(element: any) {
    ...
    const props: any = { x: element.x ?? 50, y: element.y ?? 50, draggable: true };

    // Shapes
    if (element.type === 'shape') {
      if (element.shape === 'rect') node = new Konva.Rect({ ...props, width: element.width ?? 100, height: element.height ?? 60, fill: element.color ?? '#808080' });
      else if (element.shape === 'circle') node = new Konva.Circle({ ...props, radius: element.radius ?? 40, fill: element.color ?? '#808080' });
      else if (element.shape === 'line') node = new Konva.Line({ ...props, points: element.points ?? [0, 0, 150, 0], stroke: element.color ?? 'black', strokeWidth: element.strokeWidth ?? 2 });
    }
    ...
    // Widgets
      ...
      if (element.widget === 'text') {
        ...
        if (element.placeholder) txt.setAttr('placeholder', element.placeholder);
        node = txt;
      }
      ...
      // 1D barcode generic (use JsBarcode)
        ...
        const placeholder = new Konva.Rect({ ...props, width: element.width ?? 200, height: element.height ?? 60, fill: '#eee' });
        ...
        BarcodeUtils.generateCanvas('1D', element.barcodeType ?? 'Code128', String(element.value ?? 'sample123'), (element.width ?? 200), (element.height ?? 60))
          .then(canvas => {
            const img = new Image();
            img.src = canvas.toDataURL();
            img.onload = () => {
              const kImg = new Konva.Image({ x: placeholder.x(), y: placeholder.y(), image: img, width: element.width ?? 200, height: element.height ?? 60, draggable: true });
      ...
      // QR / DataMatrix / Aztec / MaxiCode
      else if (['qrcode', 'datamatrix', 'aztec', 'maxicode', 'barcode2D'].includes(element.widget) || element.barcodeType) {
        const sizeW = element.width ?? element.size ?? 120;
        const sizeH = element.height ?? element.size ?? sizeW;

        // placeholder rect
        const placeholder = new Konva.Rect({ ...props, width: sizeW, height: sizeH, fill: '#ddd' });
        this.layer.add(placeholder);
        placeholder.dragBoundFunc(this.getDragBoundFunc(placeholder));
        this.attachNodeEvents(placeholder);
        this.widgetNodes.push(placeholder);
        ...
        const sym = element.barcodeType ?? typeMap[element.widget as BarcodeWidget] ?? 'QRCode';
        BarcodeUtils.generateCanvas('2D', sym, String(element.value ?? 'https://example.com'), sizeW, sizeH)
          .then(canvas => {
            const img = new Image();
            img.src = canvas.toDataURL();
            img.onload = () => {
              const kImg = new Konva.Image({ x: placeholder.x(), y: placeholder.y(), image: img, width: sizeW, height: sizeH, draggable: true });
      ...
      // image widget
        ...
        img.onload = () => {
          const kImg = new Konva.Image({ ...props, image: img, width: element.width ?? 100, height: element.height ?? 100, draggable: true });
          kImg.dragBoundFunc(this.getDragBoundFunc(kImg));
          this.layer.add(kImg);
          this.widgetNodes.push(kImg);
          this.layer.batchDraw();
  ...
  updateSelectedProps(props: any) {
    ...
    // Support typical attributes and barcodes
    ...
    if ((props.value !== undefined || props.barcodeType !== undefined) && node instanceof Konva.Image) {
  ...
  copy() {
    ...
    clone.position({ x: this.selectedNode.x() + 10, y: this.selectedNode.y() + 10 });
  ...
  download() {
    // Hide grid, rulers, guides, transformer temporarily
    this.gridLayer.visible(false);
    this.rulerLayer.visible(false);
    this.guideLayer.visible(false);
    this.transformer.visible(false);

    this.layer.batchDraw(); // make sure changes are applied

    // Export only the layer with paper and widgets
  ...
  private saveState() {
    const widgets = this.layer.getChildren().filter(child =>
      child !== this.paper && !(child instanceof Konva.Transformer)
    );
  ...
  private restoreFromHistory() {
    ...
    this.layer.getChildren().forEach((child: Konva.Node) => {
      if (child !== this.paper && !(child instanceof Konva.Transformer)) child.destroy();
    });
  ...
  loadFromJsonDsl(dsl: any) {
    this.resetCanvas();
    this.paperWidth = dsl.paperSize.width ?? this.paperWidth;
    this.paperHeight = dsl.paperSize.height ?? this.paperHeight;
    this.centerAndResizePaper();

    (dsl.widgets ?? []).forEach((widget: any) => this.addWidgetFromDsl(widget));
  }

  private addWidgetFromDsl(widget: any) {
    // accept variety of field names in DSL
    if (widget.type === 'text' || widget.type === 'Text') {
      this.addElement({ type: 'widget', widget: 'text', text: widget.value ?? widget.text ?? '', x: widget.position?.x ?? 20, y: widget.position?.y ?? 20, placeholder: widget.placeholder });
    } else if (widget.type === 'barcode' || widget.type === 'barcode1D' || widget.type === 'barcode2D' || widget.type === 'qrcode' || widget.type === 'datamatrix' || widget.type === 'aztec' || widget.type === 'maxicode') {
      const t = widget.barcodeType ?? (widget.type === 'qrcode' ? 'QRCode' : widget.type);
      const w = widget.size?.width ?? widget.width ?? widget.size ?? 120;
      const h = widget.size?.height ?? widget.height ?? w;
      this.addElement({ type: 'widget', widget: widget.type === 'barcode2D' ? 'datamatrix' : (widget.type || 'qrcode'), barcodeType: t, value: widget.value, x: widget.position?.x ?? 20, y: widget.position?.y ?? 20, width: w, height: h });
    } else if (widget.type === 'shape') {
      this.addElement({ type: 'shape', shape: widget.shape, x: widget.position?.x ?? 20, y: widget.position?.y ?? 20, width: widget.size?.width, height: widget.size?.height, color: widget.color });
    } else {
      // fallback - create generic rect
      this.addElement({ type: 'shape', shape: 'rect', x: widget.position?.x ?? 20, y: widget.position?.y ?? 20 });
    }
  }

  // ========== paper, rulers, grid, guides (same as before) ==========
  ...
  private drawGrid() {
    this.gridLayer = new Konva.Layer();
    const gridSize = 20;
    for (let i = 0; i < this.stage.width(); i += gridSize) this.gridLayer.add(new Konva.Line({ points: [i, 0, i, this.stage.height()], stroke: '#ddd', strokeWidth: 1 }));
    for (let j = 0; j < this.stage.height(); j += gridSize) this.gridLayer.add(new Konva.Line({ points: [0, j, this.stage.width(), j], stroke: '#ddd', strokeWidth: 1 }));
    this.stage.add(this.gridLayer);
  ...
  private drawRulers() {
    ...
    for (let x = 0; x < this.stage.width(); x += 10) {
      this.rulerLayer.add(new Konva.Line({ points: [x, 0, x, x % 50 === 0 ? 15 : 8], stroke: '#666', strokeWidth: 1 }));
      if (x % 50 === 0) this.rulerLayer.add(new Konva.Text({ x: x + 2, y: 2, text: String(x), fontSize: 10, fill: '#333' }));
    }
    for (let y = 0; y < this.stage.height(); y += 10) {
      this.rulerLayer.add(new Konva.Line({ points: [0, y, y % 50 === 0 ? 15 : 8, y], stroke: '#666', strokeWidth: 1 }));
      if (y % 50 === 0) this.rulerLayer.add(new Konva.Text({ x: 2, y: y + 2, text: String(y), fontSize: 10, fill: '#333' }));
    }
    this.stage.add(this.rulerLayer);
  ...
  private showGuides(node: Konva.Shape) {
    ...
    const snapPointsX = [this.paper.x(), this.paper.x() + this.paper.width() / 2, this.paper.x() + this.paper.width()];
    const snapPointsY = [this.paper.y(), this.paper.y() + this.paper.height() / 2, this.paper.y() + this.paper.height()];

    for (const x of snapPointsX) {
      if (Math.abs(nodeBox.x - x) < this.snapTolerance) {
        node.x(x);
        this.guideLayer.add(new Konva.Line({ points: [x, this.paper.y(), x, this.paper.y() + this.paper.height()], stroke: 'blue', strokeWidth: 1, dash: [4, 4] }));
      }
    }
    ...
    for (const y of snapPointsY) {
      if (Math.abs(nodeBox.y - y) < this.snapTolerance) {
        node.y(y);
        this.guideLayer.add(new Konva.Line({ points: [this.paper.x(), y, this.paper.x() + this.paper.width(), y], stroke: 'blue', strokeWidth: 1, dash: [4, 4] }));
      }
    }
  ...
  private attachNodeEvents(node: Konva.Node) {
    ...
    // selection event to open property panel
    node.on('click', () => {
  ...
  // ========== Backend save/load ==========
  /** Save DSL to backend. Optionally send HJSON if useHjson=true */
  saveToBackend(useHjson = false) {
    ...
    // If user wants HJSON (human friendly) you can convert here if you include hjson lib.
    // Example (if hjson installed): payload.jsonDsl = Hjson.stringify(dsl, { condense: false });
  ...
  loadFromBackend(templateId: string, versionId: string) {
    this.templateService.getTemplateVersion(templateId, versionId).subscribe((ver: any) => {
  ...
  private clampAllNodesToPaper() {
    this.layer.getChildren().forEach(child => {
      ...
      if (box.x + box.width > this.paper.x() + this.paper.width()) newX = this.paper.x() + this.paper.width() - box.width;
      if (box.y + box.height > this.paper.y() + this.paper.height()) newY = this.paper.y() + this.paper.height() - box.height;
  ...
  // ========== reset ==========
  public resetCanvas() {
    // Destroy all widget nodes and clear lists
    for (const node of this.widgetNodes) node.destroy();
    this.widgetNodes = [];

    // Destroy all other children except paper and transformer
    ...
    // DO NOT reset paperWidth and paperHeight here

    // Only reposition the paper without resizing
    this.centerPaper();
  ...
  createLine(options: any = {}) {
    return this.createNode(new Konva.Line({
  ...
  createRect(options: any = {}) {
    return this.createNode(new Konva.Rect({
  ...
  createCircle(options: any = {}) {
    return this.createNode(new Konva.Circle({
  ...
  createText(options: any = {}) {
    return this.createNode(new Konva.Text({
      x: options.x || 50, y: options.y || 50,
  ...
  createImage(options: any = {}) {
    ...
    return this.createNode(new Konva.Image({
      x: options.x || 50, y: options.y || 50,
      width: options.width || 100,
  ...
  toJsonDsl(): any {
    const dsl: any = {
      ...
      widgets: this.widgetNodes.map(node => {
        ...
        if (type === 'Text') {
          ...
          const widget = {
            type: 'text',
            value: t.text(),
            placeholder: t.getAttr('placeholder') || '',
            position: { x: t.x(), y: t.y() },
            size: { width: t.width(), height: t.height() },
            fontSize: t.fontSize(),
            color: t.fill()
          };
          if (widget.placeholder === 'templateName') dsl.templateName = widget.value;
          return widget;
        }

        // Barcode node
        if (type === 'Image' && node.getAttr('barcodeType')) {
          return {
            type: node.getAttr('is2D') ? 'barcode2D' : 'barcode1D',
            value: node.getAttr('value'),
            barcodeType: node.getAttr('barcodeType'),
